...
{
    /// <summary>
    /// Tab to display the IVs of a monster in the summary.
    /// </summary>
    public class SummaryIVTab : SummaryTab
    {
        /// <summary>
        ...
        {
            byte hpValue = monster.StatData.IndividualValues[Stat.Hp];
            HPText.SetText(hpValue.ToString());
            HPBar.value = hpValue;

            byte attackValue = monster.StatData.IndividualValues[Stat.Attack];
            AttackText.SetText(attackValue.ToString());
            AttackBar.value = attackValue;

            byte defenseValue = monster.StatData.IndividualValues[Stat.Defense];
            DefenseText.SetText(defenseValue.ToString());
            DefenseBar.value = defenseValue;

            byte specialAttackValue = monster.StatData.IndividualValues[Stat.SpecialAttack];
            SpecialAttackText.SetText(specialAttackValue.ToString());
            SpecialAttackBar.value = specialAttackValue;

            byte specialDefenseValue = monster.StatData.IndividualValues[Stat.SpecialDefense];
            SpecialDefenseText.SetText(specialDefenseValue.ToString());
            SpecialDefenseBar.value = specialDefenseValue;

            byte speedValue = monster.StatData.IndividualValues[Stat.Speed];
