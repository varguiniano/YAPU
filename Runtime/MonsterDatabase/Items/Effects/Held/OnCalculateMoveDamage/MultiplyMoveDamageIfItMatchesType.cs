...
namespace Varguiniano.YAPU.Runtime.MonsterDatabase.Items.Effects.Held.OnCalculateMoveDamage
{
    ...
    /// </summary>
    [CreateAssetMenu(menuName = "YAPU/Items/Effects/Held/OnCalculateMoveDamageWhenUsing/MultiplyMoveDamageIfItMatchesType",
                     fileName = "MultiplyMoveDamageIfItMatchesType")]
    public class MultiplyMoveDamageIfItMatchesType : OnCalculateMoveDamageItemEffect
    {
        /// <summary>
        /// Type to match.
        /// </summary>
        ...
        {
            if (move.GetMoveTypeInBattle(user, battleManager) != Type)
            {
                finished.Invoke(false, multiplier);
                yield break;
            }
